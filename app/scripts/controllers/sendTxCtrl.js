...
var sendTxCtrl = function ($scope, $sce, walletService, $rootScope) {
    ...
    $scope.$watch(() => {
        ...
        if ($scope.$airon.selectedMethod == 'eth') {
            ...
            $scope.$airon.selectedTokenObj.type = "custom";
            $scope.tx.sendMode = 'token';

            var balance = $scope.$airon.selectedTokenObj.balance;
            $scope.$airon.selectedTokenObj = new Token(
                $scope.$airon.selectedTokenObj.address,
                $scope.$airon.addressBuffer,
                $scope.$airon.selectedTokenObj.symbol,
                $scope.$airon.selectedTokenObj.decimal,
    ...
    $scope.generateTx = function () {
        ...
        if ($scope.tx.sendMode == 'token') {
            ...
            if (!isEnough($scope.tx.value, $scope.$airon.selectedTokenObj.balance)) {
                $scope.notifier.danger(globalFuncs.errorMsgs[0]);
                return;
            }

            txData.to = $scope.$airon.selectedTokenObj.getContractAddress();
            txData.data = $scope.$airon.selectedTokenObj.getData($scope.tokenTx.to, $scope.tokenTx.value).data;
            txData.value = '0x00';
        }
