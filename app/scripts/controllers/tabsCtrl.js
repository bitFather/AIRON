'use strict'
var tabsCtrl = function($scope, globalService, userService, $translate, $sce) {
/*
    $scope.onGoogleAuthBtnClick = function(element) {
        console.log(element);
...
*/
    ...
    $scope.tabNames = (tabs => {
        ...
        for (let key in tabs) {
            ...
            if (tabs[key].type == 'container') {
                for (let item in tabs[key].Items) {
                    result[item] = tabs[key].Items[item]
                }
            }
        }
        return result
    ...
    var setGasValues = function() {
        $scope.gas = {
            ...
            value: globalFuncs.localStorage.getItem(gasPriceKey, null)
                ? parseInt(globalFuncs.localStorage.getItem(gasPriceKey))
                : 41,
            max: 99,
            min: 1,
            step: 1,
    ...
    $scope.saveCustomNode = function() {
        try {
            ...
            else if (!$scope.Validator.isPositiveNumber($scope.customNode.port) && $scope.customNode.port != '')
                throw globalFuncs.errorMsgs[24]
            else if ($scope.customNode.eip155 && !$scope.Validator.isPositiveNumber($scope.customNode.chainId))
                throw globalFuncs.errorMsgs[25]
            else if (
                $scope.customNode.httpBasicAuth &&
                ($scope.customNode.httpBasicAuth.user == '' || $scope.customNode.httpBasicAuth.password == '')
            )
    ...
    $scope.setOnScrollArrows = function() {
        ...
        $scope.showRightArrow =
            document.querySelectorAll('.nav-inner')[0].clientWidth > ele.clientWidth + ele.scrollLeft
        $scope.$apply()
    }
